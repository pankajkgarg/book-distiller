...
async function callAnthropic({ apiKey, model, system, user }: { apiKey: string; model: string; system: string; user: string; }) {
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({ model, system, messages: [{ role: "user", content: [{ type: "text", text: user }] }], max_tokens: 2000, temperature: 0.7 }),
  });
...
async function callGemini({ apiKey, model, system, user }: { apiKey: string; model: string; system: string; user: string; }) {
...
export default function BookDistiller() {
  ...
  const [sections, setSections] = useState<{ id: string; content: string; heading: string; status: "draft" | "accepted" | "discarded"; order: number }[]>([]);
  const accepted = useMemo(() => sections.filter((s) => s.status === "accepted").sort((a,b)=>a.order-b.order), [sections]);
  const stitched = useMemo(() => accepted.map((s) => s.content).join("\n\n"), [accepted]);

  const transcriptRef = useRef<HTMLDivElement | null>(null);
  useEffect(() => { if (transcriptRef.current) transcriptRef.current.scrollTop = transcriptRef.current.scrollHeight; }, [sections]);
  ...
  useEffect(() => {
    (async () => {
      ...
      if (last?.value) {
        ...
        if (b) {
          setBookId(b.id); setTitle(b.title || ""); setAuthor(b.author || ""); setBookText(b.text || "");
          const secs = await db.sections.where({ bookId: b.id }).sortBy("order");
          setSections(secs);
        }
      }
    })();
  ...
  function updateSectionLocal(upd: { id: string; content?: string; heading?: string; status?: "draft" | "accepted" | "discarded"; }) {
    setSections((prev) => prev.map((s) => (s.id === upd.id ? { ...s, ...upd, heading: upd.heading ?? parseHeading(upd.content ?? s.content) } : s)));
  ...
  async function generateNext() {
    ...
    try {
      ...
      let text = "";
      if (provider === "openai") text = await callOpenAI({ apiKey, model, system, user });
      else if (provider === "anthropic") text = await callAnthropic({ apiKey, model, system, user });
      else if (provider === "google") text = await callGemini({ apiKey, model, system, user });
      else throw new Error("Unsupported provider");

      const id = crypto.randomUUID();
      const order = sections.length + 1;
      const section = { id, bookId, content: text.trim(), heading: parseHeading(text), status: "draft" as const, order };
  ...
  function accept(id: string) { updateSectionLocal({ id, status: "accepted" }); }
  ...
  function download(ext: "md" | "txt") {
    const blob = new Blob([stitched], { type: ext === "md" ? "text/markdown" : "text/plain" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url; a.download = `${(title || "distillation").toLowerCase().replace(/\s+/g, "-")}.${ext}`; a.click();
  ...
  return (
    <div className="mx-auto max-w-7xl p-4 space-y-4">
      <div className="flex items-center gap-2">
        ...
        <Badge variant="secondary" className="ml-2">Client‑only</Badge>
      </div>
      ...
      <div className="grid grid-cols-12 gap-4">
        <div className="col-span-12 lg:col-span-3 space-y-4">
          <Card>
            ...
            <CardContent className="space-y-2">
              <Input type="file" accept=".pdf,.epub,.txt,.md,.markdown" onChange={(e) => onUpload(e.target.files?.[0] || null)} />
              <div className="grid grid-cols-2 gap-2">
                <Input placeholder="Title" value={title} onChange={(e) => setTitle(e.target.value)} />
                <Input placeholder="Author" value={author} onChange={(e) => setAuthor(e.target.value)} />
              </div>
              <div className="text-xs text-muted-foreground">{charCount ? `${charCount.toLocaleString()} chars extracted` : "No text yet"}</div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <Textarea className="h-56" value={prompt} onChange={(e) => setPrompt(e.target.value)} />
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              ...
              <Input placeholder="Model (e.g., gpt-4o-mini, claude-3-5-sonnet, gemini-1.5-pro)" value={model} onChange={(e) => setModel(e.target.value)} />
              <Input type="password" placeholder="API key (stored locally)" value={apiKey} onChange={(e) => setApiKey(e.target.value)} />
              <div className="text-[11px] text-muted-foreground">Keys in the browser are visible to the page. Use temporary/restricted keys ideally.</div>
            </CardContent>
          </Card>

          <Card>
            <CardHeader><CardTitle className="text-base">Generation</CardTitle></CardHeader>
            <CardContent className="space-y-2">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2"><Switch checked={autoAdvance} onCheckedChange={setAutoAdvance} /><Label>Auto‑advance</Label></div>
                <Input className="w-20" type="number" value={maxSections} onChange={(e) => setMaxSections(parseInt(e.target.value||"1"))} />
              </div>
              <Input placeholder="Stop token" value={stopToken} onChange={(e) => setStopToken(e.target.value)} />
              <div className="flex gap-2">
                <Button className="flex-1" onClick={generateNext} disabled={isBusy}>{sections.length ? "Next" : "Start"}</Button>
                <Button variant="outline" className="flex-1" onClick={undoLast}>Undo last</Button>
              </div>
              <Button variant="outline" className="w-full" onClick={clearRun}>Clear run</Button>
              <div className="text-xs text-muted-foreground">Accepted: {accepted.length} • Total: {sections.length}</div>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="col-span-12 lg:col-span-5">
          <Card className="h-[72vh] flex flex-col">
            ...
            <CardContent className="flex-1 flex flex-col gap-2">
              <ScrollArea className="flex-1 pr-2" ref={transcriptRef}>
                <div className="space-y-3">
                  {sections.length === 0 && (
                    <div className="text-sm text-muted-foreground">No sections yet. Click <span className="font-medium">Start</span> to generate.</div>
                  )}
                  {sections.map((s, idx) => (
                    <div key={s.id} className="border rounded-xl p-3">
                      <div className="flex items-center justify-between">
                        <div className="text-sm font-semibold">{s.heading || `Section ${idx + 1}`}</div>
                        <Badge variant={s.status === "accepted" ? "default" : s.status === "discarded" ? "destructive" : "secondary"}>{s.status}</Badge>
                      </div>
                      <Textarea className="mt-2 h-40" value={s.content} onChange={(e) => edit(s.id, e.target.value)} />
                      <div className="flex gap-2 mt-2">
                        <Button onClick={() => accept(s.id)}>Accept</Button>
                        <Button variant="outline" onClick={() => discard(s.id)}>Discard</Button>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div className="col-span-12 lg:col-span-4 space-y-4">
          <Card className="h-[48vh] flex flex-col">
            ...
            <CardContent className="flex-1 overflow-auto">
              <ol className="space-y-1 text-sm">
                {accepted.length === 0 && <li className="text-muted-foreground">(empty)</li>}
                {accepted.map((s, i) => (
                  <li key={s.id} className="flex items-center gap-2">
                    <span className="w-6 h-6 inline-flex items-center justify-center rounded-full bg-foreground text-background text-xs">{i + 1}</span>
          ...
          <Card className="h-[48vh] flex flex-col">
            <CardHeader><CardTitle className="text-base">Draft (stitched)</CardTitle></CardHeader>
            <CardContent className="flex-1 overflow-auto">
              <div className="text-sm whitespace-pre-wrap leading-6 min-h-24">
                {stitched || <span className="text-muted-foreground">Accepted sections will appear here…</span>}
              </div>
              <Separator className="my-2" />
              <div className="flex gap-2">
                <Button onClick={() => navigator.clipboard.writeText(stitched)}>Copy all</Button>
                <Button variant="outline" onClick={() => download("md")}>.md</Button>
                <Button variant="outline" onClick={() => download("txt")}>.txt</Button>
              </div>
            </CardContent>
          </Card>
        </div>
      ...
      <footer className="text-xs text-muted-foreground">
        PDF/EPUB are parsed locally. Storage is persistent in your browser (IndexedDB).
      </footer>
    </div>
  );
}